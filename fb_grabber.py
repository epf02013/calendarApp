...
def updateEvents(user_id, FB_AT, prevEvents) :
    eventsFromFB=json.loads(get_fb_events(user_id, FB_AT))
    prevEvents=json.loads(prevEvents)
    for fbEvent in eventsFromFB :
        isNew=True
        for prevEvent in prevEvents :
            if fbEvent['event_id']==prevEvent['event_id'] :
                isNew=False
        if isNew :
            prevEvents.append(fbEvent)
    return prevEvents

        
        
# facebook data extract 
def get_fb_events(user_id, FB_ACCESS_TOKEN) :
